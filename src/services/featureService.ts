...
export const createFeatureRequest = async (title: string, description: string, userId: string, userEmail: string) => {
  ...
  const docRef = await addDoc(collection(db, 'feature_requests'), featureData);
  return docRef.id;
};
...
export const voteOnFeature = async (featureId: string, userId: string, voteType: 'up' | 'down' | null) => {
  const voteId = `${userId}_${featureId}`;
  ...
  await runTransaction(db, async (transaction) => {
    ...
    let downvotes = featureDoc.data().downvotes || 0;
